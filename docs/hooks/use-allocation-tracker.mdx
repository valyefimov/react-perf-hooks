...
| `options.enabled`                | `boolean`                       | No       | Enable/disable tracking in development (default `true`). Always a noop in production builds (`NODE_ENV === 'production'`), regardless of this option, so bundlers can eliminate the `FinalizationRegistry`/`WeakRef` tracking logic entirely. |
